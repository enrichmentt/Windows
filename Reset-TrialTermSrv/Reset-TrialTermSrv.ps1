$user = $env:USERNAME
$PathAclExe = "C:\windows\temp\SetACL.exe"

cmd /c "$PathAclExe -on `"hklm\SYSTEM\CurrentControlSet\Control\Terminal Server\RCM\GracePeriod`" -ot reg -actn setowner -ownr `"n:$user`""

if (($user -notlike "*SYSTEM*") -or ($user -notlike "*ALL APPLICATION PACKAGE*") -or ($user -notlike "*NETWORK SERVICE*"))
{
    cmd /c "$PathAclExe -on `"hklm\SYSTEM\CurrentControlSet\Control\Terminal Server\RCM\GracePeriod`" -ot reg -actn ace -ace `"n:$user;p:full`""
...
cmd /c "$PathAclExe -on "hklm\SYSTEM\CurrentControlSet\Control\Terminal Server\RCM\GracePeriod" -ot reg -actn setowner -ownr `"n:NETWORK SERVICE`""
